...
public class TraktV2 {
     ...
     *            the password
     */
    public void setLoginData(String username, String password) throws UnauthorizedException, LoginException {
        ...
        if (token == null || token.token == null || token.token.length() == 0) {
            throw new LoginException("Could not login - did not receive a valid token");
        }
     ...
     *
     * @param isDebug
     *            If true, the log level is set to {@link retrofit.RestAdapter.LogLevel#FULL}.
    ...
    /**
     * Create a new {@link retrofit.RestAdapter.Builder}. Override this to e.g. set your own client
     * or executor.
     *
    ...
    /**
     * Return the current {@link retrofit.RestAdapter} instance. If none exists (first call, API key
     * changed), builds a new one.
     *
     * <p>
     * When building, sets the endpoint, a {@link retrofit.RequestInterceptor} which adds the API
     * key and version headers and sets the log level.
     */
    ...
    /**
     * By default, the calendar will return all shows or movies for the specified time period. If
     * OAuth is sent, the items returned will be limited to what the user has watched, collected, or
     * added to their watchlist. You'll most likely want to send OAuth to make the calendar more
    ...
    /**
     * Checking in is a manual process used by mobile apps. While not as effortless as scrobbling,
     * checkins help fill in the gaps. You might be watching live tv, at a friend's house, or
     * watching a movie in theaters. You can simply checkin from your phone or tablet in those
     * situations.
     */
    ...
    /**
     * Comments are attached to any movie, show, season, episode, or list and can be shorter shouts
     * or more in depth reviews. Each comment can have replies and can be voted up or down. These
     * votes are used to determine popular comments.
     */
    ...
    /**
     * One or more genres are attached to all movies and shows. Some API methods allow filtering by
     * genre, so it's good to cache this list in your app.
     */
    ...
    /**
     * Recommendations are based on the watched history for a user and their friends. There are
     * other factors that go into the algorithm as well to further personalize what gets
     * recommended.
     */
    ...
    /**
     * Searches can use queries or ID lookups. Queries will search fields like the title and
     * description. ID lookups are helpful if you have an external ID and want to get the trakt ID
     * and info. This method will search for movies, shows, episodes, people, users, and lists.
     */
